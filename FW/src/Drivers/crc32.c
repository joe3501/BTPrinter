...
//const unsigned long cnCRC_32 = 0x04C10DB7; //0xEDB88320;
...
void BuildTable32( unsigned long aPoly )
{
	...
	const unsigned long		ulPolynomial = 0x04C10DB7;
	unsigned long			dwCrc;
	int						i,j;

	for(i = 0;i < 256;i++)
	{
		dwCrc = (unsigned long)i;
		for(j = 8;j > 0;j--)
		{
			if((dwCrc & 1)==1)
...
/*
unsigned short CRC(unsigned   char   *info,   DWORD   len)  
{  
	WORD   acc;  
	unsigned   char   i;  
	acc=0;  
